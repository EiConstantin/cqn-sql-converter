...
export class Transformer {
  ...
  private transformColumns(columns?: CqnColumn[]): SqlColumn[] {
    ...
    return columns.map(col => {
      ...
      if (col.ref) {
        // Handle ref: ['*'] specially (for COUNT(*))
        if (col.ref.length === 1 && col.ref[0] === '*') {
          return {
            expr: {
              type: 'func',
              name: 'COUNT',
              args: [{ type: 'ref', name: ['*'] } as SqlRef]
            } as SqlFunc,
            as: col.as
          };
        }
  ...
  private transformXpr(xpr: CqnXpr): SqlExpr {
    ...
    // Handle IS NULL (e.g., [{ ref: ['title'] }, 'is null'])
    if (items.length === 2 && items[1] === 'is null') {
      return {
        type: 'binary',
        op: 'IS NULL',
        left: this.transformExpr(items[0] as CqnExpr),
        right: { type: 'val', value: null } as SqlVal
      } as SqlBinaryOp;
    }

    // Handle IS NOT NULL (e.g., [{ ref: ['title'] }, 'is not null'])
    if (items.length === 2 && items[1] === 'is not null') {
      return {
        type: 'binary',
        op: 'IS NOT NULL',
        left: this.transformExpr(items[0] as CqnExpr),
    ...
    // Handle BETWEEN (e.g., [{ ref: ['price'] }, 'between', { val: 10 }, { val: 50 }])
    if (items.length === 4 && items[1] === 'between') {
      return {
        type: 'binary',
        op: 'BETWEEN',
        left: this.transformExpr(items[0] as CqnExpr),
        right: {
          type: 'list',
          values: [
            this.transformExpr(items[2] as CqnExpr),
            this.transformExpr(items[3] as CqnExpr)
          ]
        } as SqlList
      } as SqlBinaryOp;
    }
